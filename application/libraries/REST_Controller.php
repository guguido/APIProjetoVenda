...
abstract class REST_Controller extends CI_Controller
{
    ...
    // http://rubsphp.blogspot.com.br/2012/07/codigos-de-retorno-http.html
    ...
    public function __construct($config = 'rest')
    {
        ...
        elseif (property_exists($this, 'db')) // Use whatever database is in use (isset returns FALSE)
        {
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        elseif ($this->config->item('rest_enable_logging') && $log_method) // No key stuff, but record that stuff is happening
        {
    ...
    public function response($data = NULL, $http_code = NULL, $continue = FALSE)
    {
        ...
        if ($http_code !== NULL)
        {
            $http_code = (int) $http_code; // So as to be safe later on in the process
        }
        ...
        if ($continue === FALSE)
        {
            $this->output->_display(); // Display the data and exit execution
            exit;
        }
    ...
    protected function _detect_output_format()
    {
        ...
        if ($this->config->item('rest_ignore_http_accept') === FALSE && $http_accept !== NULL)
        {
            ...
            foreach (array_keys($this->_supported_formats) as $format)
            {
                ...
                if (strpos($http_accept, $format) !== FALSE)
                {
                    ...
                    {
                        // HTML or XML have shown up as a match. If it is truly HTML, it wont want any XML
                        return $format;
                    }
        ...
        return $this->_get_default_output_format(); // Obtain the default format from the configuration
    }
    ...
    protected function _detect_api_key()
    {
        ...
        $secret_key_variable = $this->config->item('secret_key_name'); 
        // Work out the name of the SERVER entry based on config
        $key_name    = 'HTTP_' . strtoupper(str_replace('-', '_', $api_key_variable));
        $secret_name = 'HTTP_'.strtoupper(str_replace('-', '_', $secret_key_variable)); 
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)) &&
            ($secret = isset($this->_args[$secret_key_variable]) ? $this->_args[$secret_key_variable] : $this->input->server($secret_name)))
        {
            $array = array($this->config->item('rest_key_column') . ' = ' => $key, $this->config->item('rest_secret_column') . ' = ' => $secret); 
            if (!($row = $this->rest->db->where($array)->get($this->config->item('rest_keys_table'))->row()))
            {
                return FALSE;
            }

            $this->rest->key = $row->{$this->config->item('rest_key_column')};
            $this->rest->secret_key = $row->{$this->config->item('rest_secret_column')}; 
            ...
            if (empty($row->is_private_key) === FALSE)
            {
                ...
                {
                    return FALSE; // There should be at least one IP address for this private key
                }
            }
        ...
        return FALSE; // No key has been sent
    }
    ...
    protected function _check_limit($controller_method)
    {
        ...
        elseif ($result->hour_started < (time() - $time_limit)) // Been a time limit (or by default an hour) since they called
        {
        ...
        else // They have called within the hour, so lets update
        {
    ...
    {
        $this->_get_args = array_merge($this->_get_args, $this->_query_args); // Merge both the URI segments and query parameters
    }
    ...
    protected function _parse_put()
    {
        ...
        {
           $this->_put_args = $this->input->input_stream(); // If no filetype is provided, then there are probably just arguments
        }
    }
    ...
    protected function _check_access()
    {
        ...
        $controller_method = $this->router->class . '/' . $this->router->method; // controller/method
        //check if the key has all_access
        $accessRow         = $this->rest->db
            ->where('key', $this->rest->key)
            ->where('controller', $controller_method)
            ->where('all_access', 1)
            ->get($this->config->item('rest_access_table'))->row_array();

        if (!isset($accessRow['all_access']))
        {
            return FALSE;
        }
